...
async function proxyUserRequest(
	...
	for (const prefix of BACKEND_PREFIXES) {
		try {
			const upstream = await fetch(`${BACKEND_BASE_URL}${prefix}/users/${id}`, {
				method,
				headers: {
					authorization,
	...
	return json(
		{
			...
			error: {
				...
				message: 'Nao foi possivel encaminhar a atualizacao de usuario para a API.'
			}
		},
		{ status: 500 }
	);
...
export const PATCH: RequestHandler = async ({ params, request, fetch }) => {
	...
	try {
		return proxyUserRequest(fetch, authorization, params.id, 'PATCH', await request.json());
	} catch {
		return json(
			{
				data: null,
...
export const DELETE: RequestHandler = async ({ params, request, fetch }) => {
	...
	return proxyUserRequest(fetch, authorization, params.id, 'DELETE');
};
